...
class CarbonLookasideRoute {
 ...
 public:
   ...
   *                  communicate with memcache. Note: this share_ptr is here
   *                  just to keep the router alive.
   * @param client    The CarbonRouterClient used to communicate with memcache.
   * @param prefix    Prefix prepend to memcache keys generated by the helper.
   * @param ttl       TTL of items stored in memcache by this route handle,
   *                  in seconds.
   * @param helper    The helper used to build keys and see if we should cache
   *                  a given request. This helper is use-case specific.
   */
...
typename RouterInfo::RouteHandlePtr createCarbonLookasideRoute(
  ...
  auto router = detail::createCarbonLookasideRouter(persistenceId, flavor);
  if (!router) {
