...
std::unique_ptr<folly::IOBuf> buildChain(const folly::IOBuf& data,
                                         size_t chainLength) {
...
void testCompressAndUncompress(CompressionCodec* compressor,
                               folly::IOBuf& data) {
  ...
  auto uncompressedData = compressor->uncompress(*compressedData,
                                                 data.length());
  EXPECT_EQ(data.computeChainDataLength(),
...
void testCompressTwice(CompressionCodec* compressor, folly::IOBuf& data) {
  ...
  EXPECT_EQ(compressedData->computeChainDataLength(),
            compressedData2->computeChainDataLength());
  EXPECT_EQ(compressedData->coalesce(), compressedData2->coalesce());
}

void testCompressChained(CompressionCodec* compressor,
                         folly::IOBuf& data,
                         size_t chainLength) {
  ASSERT_GT(data.computeChainDataLength(), 0);
...
void testUncompressChained(CompressionCodec* compressor,
                           folly::IOBuf& data,
                           size_t chainLength) {
  ...
  auto uncompressedData = compressor->uncompress(*compressedChainedData,
                                                 data.length());
  EXPECT_EQ(data.computeChainDataLength(),
            uncompressedData->computeChainDataLength());
  EXPECT_EQ(data.coalesce(), uncompressedData->coalesce());
