...
class LeaseTokenMap {
 public:
   ...
   * @param destination         ProxyDestination that requests with this token
   *                            should be redirected to.
   * @param destinationTimeout  Timeout of the destination.
   * @return                    Special token, that should be returned to
   *                            the client.
   */
  uint64_t insert(uint64_t originalToken,
                  std::shared_ptr<ProxyDestination> destination,
                  std::chrono::milliseconds destinationTimeout);
   ...
   * @param destination         Output parameter containing the destination
   *                            that this request should be redirected to.
   * @param destinationTimeout  Output parameter containing the timeout
   *                            of the destination.
  ...
  struct Item {
   public:
    ...
    Item(uint64_t sToken, uint64_t oToken,
         std::shared_ptr<ProxyDestination> dest,
         std::chrono::milliseconds destTimeout,
         uint32_t tokenTimeoutMs)
        : specialToken(sToken),
          originalToken(oToken),
          destination(std::move(dest)),
          destinationTimeout(destTimeout),
