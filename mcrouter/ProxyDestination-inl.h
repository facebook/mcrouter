...
void ProxyDestination<Transport>::initializeTransport() {
  ...
  transport_->setRequestStatusCallbacks(
      RequestStatusCallbacks{
          [this](int pending, int inflight) { // onStateChange
            ...
            if (inflight != 0) {
              proxy().stats().increment(
                  destination_inflight_reqs_stat, inflight);
              proxy().stats().setValue(
      ...
      ConnectionStatusCallbacks{
          [this](
            ...
            if (numConnectRetries > 0) {
              proxy().stats().increment(
                  num_connect_success_after_retrying_stat);
              proxy().stats().increment(
          ...
          [pdstnPtr = selfPtr_](
            ...
            if (reason == ConnectionDownReason::ABORTED) {
              ...
              // connection (i.e. allow remaining outstanding requests to
              // drain).
  ...
  transport_->setAuthorizationCallbacks(
