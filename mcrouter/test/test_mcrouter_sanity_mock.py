...
class TestMcrouterSanityMock(McrouterTestCase):
    ...
    def setUp(self):
        ...
        self.mcs = [
            self.add_server(self.make_memcached(), logical_port=port)
            for port in mc_ports
        ]
        self.mc_gut = self.add_server(self.make_memcached(), logical_port=mc_gut_port)
    ...
    def data(self, n):
        ...
        keys = [
            random.choice(prefixes) + randstring(random.randint(3, 10))
            for i in range(n)
        ]
        keys = list(set(keys))
